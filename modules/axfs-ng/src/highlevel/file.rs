...
impl OpenOptions {
    ...
    fn _open<M: RawMutex>(&self, context: &FsContext<M>, path: &Path) -> VfsResult<OpenResult<M>> {
        ...
        Ok(OpenResult::File(File::new(entry, flags)))
    }
...
impl<M: RawMutex> File<M> {
    pub(crate) fn new(inner: DirEntry<M>, flags: FileFlags) -> Self {
        Self {
            inner,
            flags,
            position: 0,
    ...
    pub fn metadata(&self) -> VfsResult<Metadata> {
        self.access(FileFlags::READ)?.metadata()
    }
